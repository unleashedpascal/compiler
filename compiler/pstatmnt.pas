...
implementation
    ...
    function branch_type(olddef : tdef; branchtree : tnode; var oldtree : tnode) : tdef;
      ...
      begin
        ...
        { a bare `[a, b]` branch yields a dynamic array }
        branchdef:=constructor_to_dynarray(branchtree.resultdef);
        ...
        if is_anychar(olddef) and is_string(branchdef) then
          result:=branchdef
        { char vs constant string (chararray): neither side can hold the
          other, and a chararray temp of one char triggers an IE later }
        ...
        { two constant strings: the temp takes the default string type. a
          chararray temp cannot receive a nested block of another length
          and turns into an unterminated pchar inside an array of const }
        ...
        { constant string vs shortstring: a shortstring temp holds either }
        else if (is_chararray(olddef) and is_shortstring(branchdef)) or
                (is_shortstring(olddef) and is_chararray(branchdef)) then
          result:=cshortstringtype
        else if (is_shortstring(olddef) or is_chararray(olddef)) and
                (is_ansistring(branchdef) or is_unicodestring(branchdef)) then
          result:=branchdef
    ...
    function if_statement : tnode;
      ...
      begin
        ...
        if not(current_scanner.token in endtokens) then
          if_a:=control_body_statement
        else
          if_a:=nil;

        if try_to_consume(_ELSE) then
          else_a:=control_body_statement
        else
          else_a:=nil;
    ...
    function case_statement : tnode;
      ...
      begin
        ...
        repeat
          ...
          casenode.addblock(blockid,control_body_statement);
          ...
          begin
            ...
            repeat
              ...
              if is_expr then
                begin
                  stmt:=expr(true);
                  resultdef:=branch_type(resultdef,stmt,typenode);
                end
              else
            ...
            if has_catchall and (current_scanner.token in [_ELSE,_OTHERWISE]) then
              ...
              begin
                { in expr mode `else`/`otherwise <expr>` closes with `end`,
